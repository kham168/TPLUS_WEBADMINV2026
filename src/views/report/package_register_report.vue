  ...
  <div id="RefillBalanceReport">
    <section class="section--report">
      <div class="header--report">
        <div class="title">
          <h2>{{ $t("Dashboard.package_register.title") }}</h2>
        </div>
      </div>
    </section>
    ...
    <section class="table data-promotion">
      <v-data-table
        :headers="$t('Dashboard.package_register.headers')"
        :items="DataTopUp"
        class="elevation-0"
        :footer-props="{
      ...
      >
        ...
        <template v-slot:item="{ item, index }">
          <tr>
            <td>{{ index + 1 }}</td>
            <td>
              <div v-if="item.type == 'registerPackage'">
                {{ $t("Dashboard.package_register.refill") }}
              </div>
            </td>
             <td>
              {{ item.code }}
...
export default {
  ...
  computed: {
    ...mapGetters({
      TotalBalance: "report/Package_total",
    ...
    DownloadExcel() {
      ...
      for (const i in data) {
        rows[i] = {
          ...
          code: data[i].code,
          value: data[i].value,
          register_phone: data[i].phoneDestination,
      ...
      XLSX.utils.book_append_sheet(Wb, dataWS, "Register Package");
      XLSX.writeFile(Wb, "Register_Package.xlsx");
    },
  },
};
</script>
